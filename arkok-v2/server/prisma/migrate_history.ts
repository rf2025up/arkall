...
interface LegacyChallengeParticipant {
  id: string;
  challengeId: string;
  studentId: string;
  joinedAt: Date;
...
async function createStudentIdMap(schoolId: string): Promise<Map<string, string>> {
  const studentIdMap = new Map<string, string>();

  try {
    // 从旧的Student表查询所有学生
    const legacyStudents: LegacyStudent[] = await prisma.$queryRaw`
...
async function migratePKMatches(schoolId: string, studentIdMap: Map<string, string>): Promise<number> {
  let migratedCount = 0;

  try {
    // 查询旧PK记录
    const legacyPKMatches: LegacyPKMatch[] = await prisma.$queryRaw`
    ...
    for (const legacyPK of legacyPKMatches) {
      ...
      if (legacyPK.status === 'completed') status = 'COMPLETED';
      else if (legacyPK.status === 'cancelled') status = 'CANCELLED';
...
async function migrateHabits(schoolId: string): Promise<{ habits: number; checkins: number }> {
  let migratedHabits = 0;
  let migratedCheckins = 0;

  try {
    // 迁移习惯定义
    const legacyHabits: LegacyHabit[] = await prisma.$queryRaw`
    ...
    if (habitIdMap.size > 0) {
      const legacyCheckins: LegacyHabitCheckin[] = await prisma.$queryRaw`
...
async function migrateChallenges(
  ...
  teacherId: string
): Promise<{ challenges: number; participants: number }> {
  let migratedChallenges = 0;
  let migratedParticipants = 0;

  try {
    // 迁移挑战定义
    const legacyChallenges: LegacyChallenge[] = await prisma.$queryRaw`
    ...
    for (const legacyChallenge of legacyChallenges) {
      // 转换类型和状态
      let type: 'PERSONAL' | 'GROUP' | 'CLASS' = 'PERSONAL';
      if (legacyChallenge.type === 'group') type = 'GROUP';
      else if (legacyChallenge.type === 'class') type = 'CLASS';

      let status: 'DRAFT' | 'ACTIVE' | 'COMPLETED' | 'CANCELLED' = 'ACTIVE';
      if (legacyChallenge.status === 'draft') status = 'DRAFT';
      else if (legacyChallenge.status === 'active') status = 'ACTIVE';
      else if (legacyChallenge.status === 'completed') status = 'COMPLETED';
      else if (legacyChallenge.status === 'cancelled') status = 'CANCELLED';
    ...
    if (challengeIdMap.size > 0) {
      const legacyParticipants: LegacyChallengeParticipant[] = await prisma.$queryRaw`
      ...
      for (const legacyParticipant of legacyParticipants) {
        ...
        // 转换状态和结果
        let status: 'JOINED' | 'WITHDRAWN' | 'DISQUALIFIED' = 'JOINED';
        if (legacyParticipant.status === 'withdrawn') status = 'WITHDRAWN';
        else if (legacyParticipant.status === 'disqualified') status = 'DISQUALIFIED';

        let result: 'WINNER' | 'COMPLETED' | 'FAILED' | null = null;
        if (legacyParticipant.result === 'winner') result = 'WINNER';
        else if (legacyParticipant.result === 'completed') result = 'COMPLETED';
        else if (legacyParticipant.result === 'failed') result = 'FAILED';
...
async function migrateBadges(
  ...
  studentIdMap: Map<string, string>,
  teacherId: string
): Promise<{ badges: number; studentBadges: number }> {
  let migratedBadges = 0;
  let migratedStudentBadges = 0;

  try {
    // 迁移勋章定义
    const legacyBadges: LegacyBadge[] = await prisma.$queryRaw`
    ...
    if (badgeIdMap.size > 0) {
      const legacyStudentBadges: LegacyStudentBadge[] = await prisma.$queryRaw`
