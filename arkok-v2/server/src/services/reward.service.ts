...
export class RewardService {
   ...
   * 获取学校的所有奖励配置
   */
  async getRewardConfigs(schoolId: string): Promise<RewardConfig[]> {
   ...
   * 初始化学校的默认奖励配置
   */
  async initializeDefaultConfigs(schoolId: string): Promise<RewardConfig[]> {
    const defaults = this.getDefaultConfigs();

    const configs = await Promise.all(
      defaults.map(config =>
        this.prisma.reward_configs.upsert({
          where: {
   ...
   * 创建或更新奖励配置
   */
  async upsertRewardConfig(data: CreateRewardConfigRequest): Promise<RewardConfig> {
   ...
   * 更新奖励配置
   */
  async updateRewardConfig(data: UpdateRewardConfigRequest): Promise<RewardConfig> {
   ...
   * 批量更新奖励配置
   */
  async batchUpdateRewardConfigs(
    schoolId: string,
    updates: Array<{ id: string; expReward?: number; pointsReward?: number; isActive?: boolean }>
  ): Promise<RewardConfig[]> {
    const results = await Promise.all(
      updates.map(update => this.updateRewardConfig({ id: update.id, schoolId, ...update }))
    );
   ...
   * 删除奖励配置
   */
  async deleteRewardConfig(id: string, schoolId: string): Promise<void> {
    // 验证配置是否属于该学校
    const existing = await this.prisma.reward_configs.findFirst({
      where: { id, schoolId }
    });
   ...
   * 根据模块和动作获取奖励配置
   */
  async getRewardConfig(schoolId: string, module: string, action: string): Promise<RewardConfig | null> {
   ...
   */
  async getExpForTask(schoolId: string, category: string, subcategory: string, title: string): Promise<number | null> {
    // 根据分类确定模块
    let module: string;
