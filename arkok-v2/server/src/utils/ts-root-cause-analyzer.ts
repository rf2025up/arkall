 ...
 * TypeScript 错误模式库
 */
const ERROR_PATTERNS: TSErrorPattern[] = [
  // 类型不匹配错误
  {
    regex: /error TS[0-9]+: .*? is not assignable to type .*?/,
    analyzer: (error: string) => ({
      errorType: 'type_definition_missing' as const,
  ...
  // AuthRequest 相关错误
  {
    ...
    analyzer: (error: string) => ({
      errorType: 'module_augmentation_needed' as const,
 ...
 * @returns 错误分析结果
 */
export function analyzeTSError(errorMessage: string): TSErrorAnalysis {
 ...
 * @returns 分析结果列表
 */
export function analyzeCompileErrors(compileOutput: string): TSErrorAnalysis[] {
  const errorLines = compileOutput
    .split('\n')
    .filter(line => line.includes('error TS') && line.includes('.ts'));

  const analyses: TSErrorAnalysis[] = [];
 ...
 * @returns 格式化的修复建议报告
 */
export function generateFixReport(analyses: TSErrorAnalysis[]): string {
 ...
 */
export function validateFixApproach(originalError: string, fixApproach: string): boolean {
