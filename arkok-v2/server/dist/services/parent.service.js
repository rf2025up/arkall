...
exports.parentService = exports.ParentService = void 0;
...
class ParentService {
    ...
    async login(phone, password, schoolId) {
        // 查找家长账户
        let parent = await prisma_1.default.parents.findUnique({
        ...
        // 更新最后登录时间
        await prisma_1.default.parents.update({
            where: { id: parent.id },
    ...
    async bindByInviteCode(phone, inviteCode, schoolId, studentName, name, identity) {
        ...
        // 通过学生姓名和邀请码查找学生
        const student = await prisma_1.default.students.findFirst({
            where: {
                name: studentName,
                schoolId,
        ...
        if (!student) {
            // 检查是否只是邀请码不匹配
            const studentByName = await prisma_1.default.students.findFirst({
        ...
        if (!parent) {
            parent = await prisma_1.default.parents.create({
                data: {
                    schoolId,
                    phone,
                    password: '0000', // 默认密码
        ...
        // 检查是否已绑定
        const existingBinding = await prisma_1.default.parent_student_bindings.findUnique({
        ...
        if (existingBinding) {
            ...
            // 重新激活绑定
            await prisma_1.default.parent_student_bindings.update({
            ...
            // 创建新绑定
            await prisma_1.default.parent_student_bindings.create({
                data: {
                    parentId: parent.id,
        ...
        // 绑定成功后清除邀请码（一次性使用）
        await prisma_1.default.students.update({
            where: { id: student.id },
            data: {
    ...
    async getTodayTimeline(studentId, parentId) {
        ...
        // 获取今日所有记录
        const allRecords = await prisma_1.default.task_records.findMany({
            where: {
                studentId,
        ...
        // 获取今日习惯打卡
        const habitLogs = await prisma_1.default.habit_logs.findMany({
            where: {
                studentId,
        ...
        // 获取今日PK记录
        const pkMatches = await prisma_1.default.pk_matches.findMany({
            where: {
        ...
        // 获取今日勋章
        const badges = await prisma_1.default.student_badges.findMany({
            where: {
                studentId,
        ...
        const studentProfile = await prisma_1.default.students.findUnique({
            where: { id: studentId },
        ...
        // 获取今日点赞和留言状态
        const summary = await prisma_1.default.daily_summaries.findFirst({
            where: {
                studentId,
                parentId,
    ...
    async getHistoryTimeline(studentId, parentId, page = 1, limit = 10) {
        ...
        const records = await prisma_1.default.task_records.findMany({
            where: { studentId },
    ...
    async likeToday(studentId, parentId) {
        ...
        await prisma_1.default.daily_summaries.upsert({
            where: {
    ...
    async verifyParentAccess(parentId, studentId) {
        const binding = await prisma_1.default.parent_student_bindings.findFirst({
            where: {
                parentId,
                studentId,
                isActive: true
     ...
     * 构建时间轴数据
     */
    buildTimeline(records, habitLogs, pkMatches, badges, studentId) {
        const timeline = [];
        // 分离 QC 记录和其他记录
        const qcRecords = [];
        const otherRecords = [];
 ...
 */
    ...
    async getStudentParents(studentId) {
        const bindings = await prisma_1.default.parent_student_bindings.findMany({
    ...
    async unbindParent(bindingId) {
        const binding = await prisma_1.default.parent_student_bindings.findUnique({
            where: { id: bindingId },
            include: {
    ...
    async getTeacherFeedbacks(schoolId, unreadOnly = false) {
        ...
        const summaries = await prisma_1.default.daily_summaries.findMany({
            where,
    ...
    async markFeedbackRead(feedbackId) {
        await prisma_1.default.daily_summaries.update({
            where: { id: feedbackId },
    ...
    async markAllFeedbacksRead(schoolId) {
        await prisma_1.default.daily_summaries.updateMany({
            where: {
                teacherRead: false,
                students: { schoolId }
            },
    ...
    async calculateRadarStats(studentId) {
        ...
        // 1. 学业攻克：QC 完成率
        const qcStats = await prisma_1.default.task_records.groupBy({
            by: ['status'],
            where: {
                studentId,
                type: 'QC'
        ...
        const monthlyTasks = await prisma_1.default.task_records.count({
            where: {
                studentId,
                type: 'TASK',
                status: 'COMPLETED',
        ...
        // 5. 荣誉成就：勋章数量（归一化）
        const badgeCount = await prisma_1.default.student_badges.count({
            where: { studentId }
        });
    ...
    async getMonthlyHeatmap(studentId) {
        ...
        const [taskRecords, habitLogs] = await Promise.all([
            prisma_1.default.task_records.findMany({
                where: {
                    studentId,
    ...
    async getGrowthSummary(studentId) {
        ...
        const [totalTasks, totalQC, totalPK, totalHabits, totalBadges] = await Promise.all([
            prisma_1.default.task_records.count({ where: { studentId, type: 'TASK', status: 'COMPLETED' } }),
            prisma_1.default.task_records.count({ where: { studentId, type: 'QC', status: 'COMPLETED' } }),
            prisma_1.default.pk_matches.count({ where: { OR: [{ studentA: studentId }, { studentB: studentId }] } }),
            prisma_1.default.habit_logs.count({ where: { studentId } }),
            prisma_1.default.student_badges.count({ where: { studentId } })
        ]);
        // 计算入学天数
