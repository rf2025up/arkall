...
// 获取教师的1v1教学计划列表
router.get('/', auth_middleware_1.authenticateToken, auth_middleware_1.requireTeacher, async (req, res) => {
    try {
        ...
        const plans = await tutoringService.getTeacherTutoringPlans(teacherId, {
            status: status,
            dateRange: dateRange ? {
...
// 创建1v1教学计划
router.post('/', auth_middleware_1.authenticateToken, auth_middleware_1.requireTeacher, async (req, res) => {
    try {
...
// 更新教学计划状态
router.patch('/:id/status', auth_middleware_1.authenticateToken, auth_middleware_1.requireTeacher, async (req, res) => {
    try {
        const { teacherId } = req.user;
        const { id } = req.params;
        const updates = req.body;
        const tutoringService = getPersonalizedTutoringService();
        const plan = await tutoringService.updateTutoringPlanStatus(id, teacherId, updates);
        res.json({ success: true, data: plan });
    }
    catch (error) {
...
// 删除教学计划
router.delete('/:id', auth_middleware_1.authenticateToken, auth_middleware_1.requireTeacher, async (req, res) => {
    try {
        const { teacherId } = req.user;
        const { id } = req.params;
        const tutoringService = getPersonalizedTutoringService();
        await tutoringService.deleteTutoringPlan(id, teacherId);
...
router.get('/download-record', auth_middleware_1.authenticateToken, auth_middleware_1.requireTeacher, async (req, res) => {
    try {
        const { teacherId, schoolId } = req.user;
...
// ✅ 宪法合规：生成Excel的独立函数
async function generateTutoringRecordsExcel(records) {
...
function formatTutoringMethods(methods) {
